...
def convert_me(dfSalesDaysFuture, dfSalesMonthly, dfReinvestmentProjects, dfCannibalization, dfReinvestFactors, strScope="forecast"):
    dfSalesDaysFuture = dfSalesDaysFuture.filter(
        F.col("open_date").isNotNull() & F.col("close_date").isNotNull()
    )
    ...
    dfFuture.coalesce(1).write.mode("overwrite").csv("/Users/riddhi_gohil/Desktop/personal_git_repos/r-to-pyspark/output/super_optmised_temp", header=True)
    return dfFuture
