...
def fun_fill_day_details(df_sales_monthly):
    ...
    df_main = df_with_latest.groupBy("jan1_day", "month_num", "leap_year", "year_ref") \
        .agg(F.expr("percentile_approx(days, 0.5)").cast("int").alias("days_max"))

    df_manual_add_1 = df_main.filter(
        (col("month_num") <= 2) & (~col("leap_year")) & (col("jan1_day") == "Wednesday")
    ).withColumn("leap_year", lit(True)) \
     .withColumn("days_max", col("days_max") + when(col("month_num") == 2, 1).otherwise(0))

    df_manual_add_2 = df_main.filter(
        (col("month_num") > 2) & (~col("leap_year")) & (col("jan1_day") == "Thursday")
    ).withColumn("leap_year", lit(True)) \
     .withColumn("jan1_day", lit("Wednesday"))

    df_manual_combined = df_manual_add_1.union(df_manual_add_2)

    df_manual_final = df_manual_combined.join(df_main, ["jan1_day", "month_num", "leap_year"], "anti")
...
def fun_sales_days_between(start_col, end_col):
    return size(expr(f"filter(sequence({start_col}, {end_col}, interval 1 day), x -> date_format(x, 'EEEE') != 'Sunday')"))

def fun_sales_days_between(start_col, end_col):
    return size(
        F.filter(
            sequence(start_col, end_col),
            lambda x: dayofweek(x) != 1  # Sunday = 1
        )
    )
