...
def convert_me(dfSalesDaysFuture, dfSalesMonthly, dfReinvestmentProjects, 
               dfCannibalization, dfReinvestFactors, strScope="forecast"):
    ...
    dfSalesDaysFuture = dfSalesDaysFuture.withColumn(
        "nMonthStart",
        F.when(F.dayofmonth("date_forecast") < F.lit(actual_cutoff), F.lit(-1)).otherwise(F.lit(0))
    ...
    dfSeq = dfSalesDaysFuture.withColumn(
        "month_offset_arr",
        F.sequence(F.col("nMonthStart"), F.col("months_predict") - 1)
    ).withColumn(
        "offset", F.explode("month_offset_arr")
    ).drop("month_offset_arr")
    dfSeq = dfSeq.withColumn("month", F.add_months(F.trunc("date_forecast", "month"), F.col("offset"))).drop("offset")
    dfSeq = dfSeq.filter((F.col("month") >= F.col("month_open")) & (F.col("month") <= F.col("month_close")))
    dfSeq = dfSeq.withColumn("concept_code", F.col("location_type_code")) \
             .withColumn("location_type_code", F.col("location_type_code")) \
             .withColumn("price_group", F.col("price_group"))
    dfSeq = dfSeq.withColumn("jan1_day", F.date_format(F.trunc("month", "year"), "EEEE")) \
    ...
    dfHist = dfHist.withColumn("nMonthsHistory", F.count("*").over(Window.partitionBy("loc_num"))) \
                   .withColumn("rn", F.row_number().over(window_loc)) \
                   .filter((F.col("nMonthsHistory") >= 12) & (F.col("rn") >= F.col("nMonthsHistory") - 12 + 1)) \
                   .select("loc_num", "month_num", "days").dropDuplicates(["loc_num", "month_num"])
    dfFuture = dfFuture.withColumn("month_num", F.month("month"))
    dfFuture = dfFuture.join(dfHist, on=["loc_num", "month_num"], how="left")
    ...
    if dfFuture.filter(F.col("location_type_code").isNull() | F.col("concept_code").isNull()).count() > 0:
        raise Exception("Missing location type or concept codes")
    ...
    dfFuture = dfFuture.fillna({"days_lost": 0, "days_before_open": 0, "days_after_close": 0})
    dfFuture = dfFuture.withColumn("days_max", F.col("days_max") - F.col("days_lost"))
    dfFuture = dfFuture.withColumn(
        "days_max",
        F.when(F.col("month") == F.col("month_open"), F.col("days_max") - (F.col("days_before_open") - 1))
         .otherwise(F.col("days_max"))
    ).withColumn(
        "days_max",
        F.when(F.col("month") == F.col("month_close"), F.col("days_max") - F.col("days_after_close"))
         .otherwise(F.col("days_max"))
    )
    dfFuture = dfFuture.withColumn("days_max", F.greatest(F.lit(0), F.col("days_max")))
    dfFuture = dfFuture.withColumn("age", fun_months_between_exact("month_open", "month")) \
                       .withColumnRenamed("days_max", "days") \
                       .drop("days_lost", "open_date", "close_date", "month_open", "month_close",
                             "days_before_open", "days_after_close", "month_num")
    dfFuture = dfFuture.withColumn(
        "reported_month",
    ...
    dfInfl = dfSalesMonthly.select(
        "loc_num",
        F.col("month").alias("reported_month"),
    ...
    dfInc = dfInc.withColumn(
        ...
        F.when(F.col("month") < F.lit("2025-05-01").cast("date"), F.lit(1.0))
         .otherwise(F.lit(fun_annualized_inflation(strScope == "forecast")))
    )
    dfInc = dfInc.withColumn("begin_inflation_rate_monthly", F.pow(F.col("begin_inflation_rate"), 1/12))
    dfInc = dfInc.withColumn(
        "begin_inflation_rate_monthly",
        F.when(F.col("months_out") == 0, F.lit(1.0)).otherwise(F.col("begin_inflation_rate_monthly"))
    )
    dfInc = dfInc.withColumn("end_inflation_rate", F.lit(fun_long_run_inflation()))
    dfInc = dfInc.withColumn("slope", (F.col("begin_inflation_rate") - F.col("end_inflation_rate")) / 12)
    dfInc = dfInc.withColumn("M", fun_months_between_exact("date_forecast", F.lit("2026-01-01").cast("date")))
    dfInc = dfInc.withColumn("intermediate_inflation_rate",
                             F.col("begin_inflation_rate") + F.col("slope") * (F.col("M") - F.col("months_out")))
    dfInc = dfInc.withColumn(
        "intermediate_inflation_rate",
        F.when(F.col("intermediate_inflation_rate") < F.col("begin_inflation_rate"),
               F.col("begin_inflation_rate")).otherwise(F.col("intermediate_inflation_rate"))
    )
    dfInc = dfInc.withColumn(
        "intermediate_inflation_rate",
        F.when(F.col("intermediate_inflation_rate") > F.col("end_inflation_rate"),
               F.col("end_inflation_rate")).otherwise(F.col("intermediate_inflation_rate"))
    )
    dfInc = dfInc.withColumn("intermediate_inflation_rate_monthly", F.pow(F.col("intermediate_inflation_rate"), 1/12))
    dfInc = dfInc.withColumn(
        "intermediate_inflation_rate_monthly",
        F.when(F.col("months_out") == 0, F.lit(1.0)).otherwise(F.col("intermediate_inflation_rate_monthly"))
    )
    ...
    # added for similarity
    dfFuture = dfFuture.withColumn("gocf", F.round("gocf", 15))
    ...
    dfFuture = dfFuture.withColumn("age_years", F.round(F.col("age") / 12, 1))
    dfFuture = dfFuture.withColumn(
        "age_group",
