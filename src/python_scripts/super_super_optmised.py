...
def convert_me(dfSalesDaysFuture, dfSalesMonthly, dfReinvestmentProjects, dfCannibalization, dfReinvestFactors, strScope="forecast"):
    # Early aggressive filtering to minimize shuffle later
    ...
    dfFuture = dfFuture.repartition(10).orderBy("loc_num").select(final_cols)
    dfFuture.coalesce(1).write.mode("overwrite").csv("/Users/riddhi_gohil/Desktop/personal_git_repos/r-to-pyspark/output/super_super_optmised_temp", header=True)
