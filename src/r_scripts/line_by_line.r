...
dfCannibalization <- read.csv("~/Desktop/Candidate Assessment/data/dfCannibalizationFactors.csv")
dfSalesMonthly <- read.csv("~/Desktop/Candidate Assessment/data/dfMonthlySales.csv")
dfReinvestFactors <- read.csv("~/Desktop/Candidate Assessment/data/dfReinvestmentFactors.csv")
...
dfCannibalization <- dfCannibalization %>% mutate(month = ymd(month))

# dfMonthlySales cleaning
dfSalesMonthly <- dfSalesMonthly %>% mutate(
  loc_num = str_pad(loc_num, 5, pad = "0"),
  month = ymd(month), 
  open_date = ymd(open_date)
)

# dfReinvestFactors cleaning
dfReinvestFactors <- dfReinvestFactors %>% mutate(
  loc_num = str_pad(loc_num, 5, pad = "0"),
  month = ymd(month))
